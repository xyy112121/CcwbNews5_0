...
{
    if (!_voiceView) {
        _voiceView = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetMaxX(self.voiceButton.frame) + 5, 7, MTScreenW - 85, kInputHeight)];
...
-(void)setupSubviews
{
    ...
    self.textInput = [[UITextView alloc] initWithFrame:CGRectMake(CGRectGetMaxX(self.voiceButton.frame) + 5, (self.height - kInputHeight)/2, MTScreenW - 85, 37)];
...
//    [_buttonArr addObject:self.emojiButton];
    ...
    self.moreButton = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetMaxX(self.textInput.frame) + 5, self.height - kButtonWH - kButtonMargin, kButtonWH, kButtonWH)];
    ...
    [self.moreButton setImage:[UIImage imageNamed:@"liaotian_ic_gengduo_nor"] forState:UIControlStateSelected];
...
- (void)keyboardWillHidden:(NSNotification *)notification
{
	...
	NSLog(@"self.frame.hide====%f",self.y);
}
...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    if (self.textInput.scrollEnabled) {
        ...
        self.y = MTScreenH - _keyboardHeight - _textInputMaxHeight - 5 - 8-64;
        ...
        self.y = MTScreenH - _keyboardHeight - _textInputHeight - 5 - 8-64;
...
- (void)voiceButtonTouchDown
{
    ...
    //录音
    [MTRecordHelper shareRecordHelper].recordEndBlock = ^(NSString *mp3filepath){
        NSLog(@"录音完成");
        
        if ([_delegate respondsToSelector:@selector(inputToolbar:sendRecordData:)]) {
            [_delegate inputToolbar:self sendRecordData:mp3filepath];
        }
    };
