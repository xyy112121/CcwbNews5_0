...
- (void) addTitleSource:(NSMutableArray *)titleSource bottomLineColor:(UIColor*)bottomLineColor{
    ...
    // 计算按钮的间距
	CGFloat padding = 20;//(self.bounds.size.width - fontWith) / (seugemtCount + 1); // 30
    ...
    for (NSInteger i = 0; i < seugemtCount; i++) {
        ...
        CGFloat btnX = i  * padding + frontBtnLength+(self.frame.size.width-93-40)/2;
		
		DLog(@"current====%f,%f",currentBtnLenth,self.frame.size.width);
        // 按钮的frame存储在frame中
    ...
    // 创建指示器
    self.bottomLine =[[UIView alloc]initWithFrame:CGRectMake([[self.btnFrames valueForKey:@"0"] CGRectValue].origin.x, self.bounds.size.height - BOTTOMLIINEHEIGHT, [[self.btnFrames valueForKey:@"0"] CGRectValue].size.width, BOTTOMLIINEHEIGHT)];
