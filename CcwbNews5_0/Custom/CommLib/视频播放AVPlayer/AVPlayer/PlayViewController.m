...
- (void)viewDidLoad
{
    ...
    self.view.layer.transform=CATransform3DMakeRotation(M_PI_2, 0, 0, 1);
...
-(void)createUI
{
    _playBtn=[[UIButton alloc]initWithFrame:CGRectMake(50, SCREEN_WIDTH-60, 60, 40)];
    ...
    _playBtn.titleLabel.font=[UIFont boldSystemFontOfSize:14];
    [_playBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateSelected];
    [_playBtn addTarget:self action:@selector(playAction:) forControlEvents:UIControlEventTouchUpInside];
    _playBtn.backgroundColor=[UIColor clearColor];
    [self.view  addSubview:_playBtn];
    _progress=[[UIProgressView alloc]initWithFrame:CGRectMake(110, _playBtn.frame.origin.y+21, kHeight-230, 2)];
    //    设置轨道颜色，将要走到的路径的颜色
    _progress.trackTintColor=[UIColor whiteColor];
    //    设置进度颜色,已经走过的路径
    _progress.progressTintColor=[UIColor grayColor];
    
    _slider=[[UISlider alloc]initWithFrame:CGRectMake(110, _playBtn.frame.origin.y+6,kHeight-230, 30)];
    ...
    _timeLabel=[Tools createLabelWithFrame:CGRectMake(kHeight-130, _playBtn.frame.origin.y+10, 110, 20) text:nil textColor:[UIColor whiteColor] textAligment:NSTextAlignmentRight andBgColor:[UIColor clearColor] font:[UIFont systemFontOfSize:14]];
    _timeLabel.text=@"--:--/--:--";
...
-(void)createTitleView
{
	...
	backBtn.titleLabel.font = FONTN(15.0f);
